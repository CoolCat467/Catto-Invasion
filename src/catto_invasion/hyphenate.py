...
class Hyphenator:
    ...
    def __init__(self, patterns: str, exceptions: str = "") -> None:
        ...
        self.tree: dict[
            str | None,
            dict[str | None, list[int] | Any] | list[int],
        ] = {}
        ...
        for ex in exceptions.split():
            ...
            self.exceptions[ex.replace("-", "")] = [0] + [
                int(h == "-") for h in re.split(r"[a-z]", ex)
            ]

    def _insert_pattern(self, pattern: str) -> None:
        """Insert pattern into pattern tree."""
        ...
        for c in chars:
            if c not in t:
                t[c] = {}
            t = cast(
                "dict[str | None, dict[str | None, list[int] | Any] | list[int]]",
                t[c],
            )
        t[None] = points
        ...
        if word.lower() in self.exceptions:
            ...
            for i in range(len(work)):
                ...
                for c in work[i:]:
                    ...
                    if None in t:
                        p: list[int] = cast("list[int]", t[None])
                        for j, point in enumerate(p):
                            points[i + j] = max(points[i + j], point)
            # No hyphens in the first two chars or the last two.
            points[1] = points[2] = points[-2] = points[-3] = 0

        # Examine the points to build the pieces list.
        pieces = [""]
        for c, point in zip(word, points[2:], strict=False):
            pieces[-1] += c
            if point % 2:
                pieces.append("")
        return pieces
...
ss5w 2st. s2tag s2tal stam4i 5stand s4ta4p 5stat. s4ted stern5i s5tero ste2w"""  # codespell:ignore 2st
